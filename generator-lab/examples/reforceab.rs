//! Interleaved A/B of the hidden-single re-force fast path on the production SIMT
//! stream: variant A = fast path OFF (the old gate sequence), variant B = ON. The two
//! [`PuzzleStream`]s run over the SAME seeds in alternating order-swapped chunks inside
//! one binary (the drift-proof protocol), and because the fast path is
//! trajectory-invariant the two variants MUST produce identical puzzle sets — the fp
//! equality printed at the end is the soundness guard, the time ratio is the verdict.
//!
//! Spec selection is combobench's: --force NAME[:COUNT] (repeatable), --toolbox full.
//!
//! Usage:
//!   cargo run --release -p generator-lab --example reforceab -- \
...
fn main() {
    ...
    let label = forces
        ...
        .join(" + ");
    println!(
        "reforceab[{label}] toolbox={}: {per_variant}/variant in {chunk}-attempt chunks, seed {base_seed}",
        if toolbox_full { "full" } else { "train-union" },
    );

    let mut a = PuzzleStream::new_with(base_seed.., &spec, false); // A: fast path OFF
    let mut b = PuzzleStream::new_with(base_seed.., &spec, true); // B: fast path ON
    ...
    // Soundness guard: the fast path is trajectory-invariant, so the seed -> puzzle
    // map must be IDENTICAL. The two variants overshoot the attempt budget on
    // different tick boundaries, so compare over the seeds BOTH completed.
