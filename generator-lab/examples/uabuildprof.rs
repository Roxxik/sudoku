...
//! `UaTier::Full` library over it many times, so `random_solution`'s fill amortizes out and
...
use generator_lab::generate::{UaTier, ua_build_cost_pooled};
...
fn main() {
    ...
    let (nanos, uas) = ua_build_cost_pooled(seed, boards, repeats, UaTier::Full);
