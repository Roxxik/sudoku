//! Interleaved A/B of the SIMT stream host implementation: variant A = the
//! hand-rolled state machine ([`random_simt::PuzzleStream`]), variant B = the
//! coroutine host ([`random_simt_co::PuzzleStream`], lanes as `std::ops::Coroutine`).
//! Same warp kernel, same seeds, alternating order-swapped chunks in one binary (the
//! drift-proof protocol); both hosts walk identical gate sequences, so the per-seed
//! puzzle comparison printed at the end is the soundness guard, the time ratio the
//! verdict.
//!
...
//!
//! Usage:
//!   cargo run --release -p generator-lab --example coroutineab -- \
//!       [--force NAME[:COUNT] ...] [--toolbox train|full] \
//!       [--per-variant 40000] [--chunk 2000] [--seed 1]

use generator_lab::generate::random_simt::{Pumped, PuzzleStream as SmStream};
use generator_lab::generate::random_simt_co::PuzzleStream as CoStream;
use generator_lab::spec::Spec;
use generator_lab::spec::kinds::{DIFFICULTY, HIDDEN_QUAD, NAMES, NUM, Tier, branch_of, tier_of};

/// Pump the state-machine stream until its cumulative attempt counter reaches
/// `target`, recording each found puzzle's `(seed, fp)`; returns the wall time.
fn run_chunk_sm<I: Iterator<Item = u64>>(
    s: &mut SmStream<I>,
    target: usize,
...
/// Same, for the coroutine stream.
fn run_chunk_co<I: Iterator<Item = u64>>(
    s: &mut CoStream<I>,
    target: usize,
    found: &mut Vec<(u64, u64)>,
) -> f64 {
...
fn main() {
    ...
    let label = forces
        ...
        .join(" + ");
    println!(
        "coroutineab[{label}] toolbox={}: {per_variant}/variant in {chunk}-attempt chunks, seed {base_seed}",
        if toolbox_full { "full" } else { "train-union" },
    );

    let mut a = SmStream::new(base_seed.., &spec); // A: hand-rolled state machine
    let mut b = CoStream::new(base_seed.., &spec); // B: coroutine lanes
    ...
    for c in 0..chunks {
        ...
        let mut run = |which: u8| {
            let (dt, before, after) = if which == 0 {
                ...
                let dt = run_chunk_sm(&mut a, target, &mut found_a);
                (dt, before, a.stats().attempts)
            } else {
                let before = b.stats().attempts;
                let dt = run_chunk_co(&mut b, target, &mut found_b);
    ...
    println!("  A (state machine): {us_a:>7.2} us/att   yield {} / {}", sa.successes, sa.attempts);
    println!("  B (coroutine):     {us_b:>7.2} us/att   yield {} / {}", sb.successes, sb.attempts);
    println!(
