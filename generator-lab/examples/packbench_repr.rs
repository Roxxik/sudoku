...
//!
//! The new-`repr` twin of `packbench` (which benches the old bb-based warp). Run both to
//! confirm the port did not regress the warp's per-attempt cost vs the bb stack.
//!
//! Usage: cargo run --release -p generator-lab --example packbench_repr -- [lanes=8] [per_lane=4000]
...
fn main() {
    ...
    println!("packbench_repr: {lanes} lanes x {per_lane} attempts = {total} total attempts/mode\n");
