//! Interleaved A/B of the cross-stall subset-ladder memo on the production SIMT
//! stream: variant A = memo OFF (full rescan per stall), variant B = ON. Both run the
//! hidden-single re-force fast path (production baseline). The two [`PuzzleStream`]s
//! run over the SAME seeds in alternating order-swapped chunks inside one binary (the
//! drift-proof protocol); the memo is exact (first-fire-preserving), so the per-seed
//! puzzle comparison printed at the end is the soundness guard, the time ratio the
//! verdict. With `--features count`, prints the memo's unit-scan skip share.
//!
...
fn run_chunk<I: Iterator<Item = u64>>(
    s: &mut PuzzleStream<I>,
    target: usize,
...
fn main() {
    ...
    let mut a = PuzzleStream::new_opts(base_seed.., &spec, true, false); // A: memo OFF
    let mut b = PuzzleStream::new_opts(base_seed.., &spec, true, true); // B: memo ON
    ...
    // Soundness guard: the memo is exact, so the seed -> puzzle map must be IDENTICAL.
    // The two variants overshoot the attempt budget on different tick boundaries, so
    // compare over the seeds BOTH completed.
    ...
    println!("  A (off): {us_a:>7.2} us/att   yield {} / {}", sa.successes, sa.attempts);
    println!("  B (on):  {us_b:>7.2} us/att   yield {} / {}", sb.successes, sb.attempts);
    println!(
    ...
    // Memo bite (both variants pooled — A contributes only scans, B scans + skips).
    #[cfg(feature = "count")]
    {
        let l = generator_lab::solve::lstat_snapshot();
        println!(
            "  ladder: {} entries, unit-scans {} run / {} skipped ({:.1}% skip share)",
            l[0],
            l[1],
            l[2],
            100.0 * l[2] as f64 / (l[1] + l[2]).max(1) as f64,
        );
    }
}
