...
//! warp ([`run_warp`], packed prober + scalar per-lane baseline) against the two surviving
//! hosts that also vectorize the baseline:
//!   - **batched fire-at-8** — [`run_warp_simt`]: a [`PackedProber`] feeds a deferred
//!     [`PackedSolver`] batch fired the instant 8 unique gates accumulate. Needs lanes >> 8
//!     (oversubscription) to keep the baseline warp fed.
...
//!
//! Correctness: each host is cross-checked against `run_warp` at a matched lane count
...
fn main() {
    ...
    for (mode, label) in [(0u32, "train"), (1u32, "drill")] {
        ...
        // Correctness: hosts match run_warp at a matched (small) lane count.
        check_match("simt", &spec, 16, 2000);
