//! Interleaved A/B of the warp-merge step on the coroutine host: variant A = the
//! two-struct host ([`random_simt_co::PuzzleStream`], UnifiedWarp + event buffer),
//! variant B = the merged host ([`random_simt_merged::PuzzleStream`], slot lifecycle
//! inline, no event buffer). Same lane coroutines, same kernels, same seeds,
//! alternating order-swapped chunks in one binary; the per-seed puzzle comparison is
//! the soundness guard, the time ratio the verdict.
//!
//! Spec selection is combobench's: --force NAME[:COUNT] (repeatable), --toolbox
//! train|full. No --force defaults to train(HiddenQuad).
//!
//! Usage:
//!   cargo run --release -p generator-lab --example warpmergeab -- \
...
/// Pump the two-struct stream until its cumulative attempt counter reaches
...
fn main() {
    ...
    let label = forces
        ...
        .join(" + ");
    println!(
        "warpmergeab[{label}] toolbox={}: {per_variant}/variant in {chunk}-attempt chunks, seed {base_seed}",
        if toolbox_full { "full" } else { "train-union" },
    );

    let mut a = CoStream::new(base_seed.., &spec); // A: two-struct (warp + events)
    let mut b = MergedStream::new(base_seed.., &spec); // B: merged (inline slots)
    ...
    for c in 0..chunks {
        ...
        let mut run = |which: u8| {
            let (dt, before, after) = if which == 0 {
                ...
                let dt = run_chunk_co(&mut a, target, &mut found_a);
                (dt, before, a.stats().attempts)
            } else {
                let before = b.stats().attempts;
                let dt = run_chunk_merged(&mut b, target, &mut found_b);
    ...
    println!("  A (warp+events): {us_a:>7.2} us/att   yield {} / {}", sa.successes, sa.attempts);
    println!("  B (merged):      {us_b:>7.2} us/att   yield {} / {}", sb.successes, sb.attempts);
    println!(
