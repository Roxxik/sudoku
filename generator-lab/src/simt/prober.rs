...
//! the next probe (the perfect-refill regime `packsim`/`warpsim` predicted,
//! `killswitch` realized at ~2.6x per core).
//!
...
//!
//! Verdicts are identical to the scalar prober (existence is order- and
//! technique-independent), so the warp stays byte-identical to the sequential
//! generator (the equivalence test pins it).

use crate::bb::{rm_bit, rm_cell, rm_lane};
