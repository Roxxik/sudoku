...
// kindIndex (usage-array position) -> kebab id. The worker spec is keyed by the
// technique's stable kebab id, not by kindIndex, so the JS kindIndex ordering never
// crosses the wasm boundary (see web/curriculum.js).
const ID_BY_KIND = (() => {
  const m = {};
  for (const t of CURRICULUM) m[t.kindIndex] = t.id;
  return m;
})();

// Translate a positional usage array (indexed by kindIndex; see spec.js) into the
// id-keyed spec object the worker maps back to its own kinds via `kinds::NAMES`.
// Only non-Off entries are emitted; off/unknown kinds are simply absent.
function specFromUsages(usages) {
  const spec = {};
  for (let i = 0; i < usages.length; i++) {
    if (usages[i] === 0) continue; // OFF
    const id = ID_BY_KIND[i];
    if (id) spec[id] = usages[i];
  }
  return spec;
}
...
// Generate one puzzle. Every request carries `usages` (the per-kind usage array
// from spec.js): campaign builds it via campaign.js, Custom/Review/Daily via their
// own builders. It is translated to the worker's id-keyed `spec` field, so the JS
// kindIndex ordering never crosses the boundary. `forceAny` folds the Forced kinds
// into one disjunction -- the puzzle requires some member, not every one (Review /
// Daily). `uncapped` lifts the worker's attempt budget so the search runs until it
// succeeds or is cancelled (the page offers this after a capped attempt gives up).
// `seed` (a u64 decimal string) pins the RNG so every device generates the same
// puzzle -- the daily puzzle sends a per-(date, difficulty) seed; ordinary
// generation omits it and the worker draws its own. Resolves to
// { puzzle, solution, givens, seed, attempts, grade } (seed is a decimal-string
// u64); rejects on a generation failure (capped budget exhausted) or if cancelled.
export function generate({ usages, uncapped, forceAny, seed }) {
  if (pending) return Promise.reject(new Error("a generation is already running"));
  if (!worker) spawn();
  const msg = { spec: specFromUsages(usages), uncapped: !!uncapped, forceAny: !!forceAny };
  // A pinned seed is sent only when present, so ordinary requests let the worker
  // draw its own seed.
