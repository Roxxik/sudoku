...
const HELP_TOUCH = [
  ...
  ["Switch note style", "Double-tap or swip the Notes button to the left or right to swap between center notes and corner (Snyder) notes."],
...
const HELP_NOTES = [
  ...
  ["Your hints are counted", "Every puzzle keeps a hint tally — “hint-free” or a count, shown by the Hint button, on the Continue cards, and in your Stats. Two things add to it. The plain one: asking the hints to reveal where a technique goes. The easy-to-miss one: the mistake check telling you an entry can't be right against the solution — a placed digit that clashes with nothing on the board yet still isn't the answer, or a pencil mark that has ruled out a cell's true digit. That one costs a hint because only the solution can reveal it. Free, by contrast: a contradiction you could find yourself — the same digit twice in a row, column, or box — a board that comes back all clear, spotting or clearing stale marks, a second look at a hint you've already used, and a technique's name alone (“there's a Hidden Single”) without revealing where it goes."],
