//! Variant `banded-nolc`: [`banded`](super::banded) with locked candidates
//! removed from the existence search (naked + hidden singles in both views only).
//!
//! The prober is a pure existence oracle, so a prune-only technique like LC is
//! optional — completeness comes from branching. This isolates LC's net value in
//! the dual-view prober: it forgoes LC's pruning (a few % more DFS nodes) but skips
//! the `triplet_occ` + `BAND_KEEP_OCC` lookup on every (band,digit) scan. Together
//! with `banded` / `banded-sl` / `banded-sl-nolc` it forms the 2x2 {dual,single} x
//! {LC,no-LC}. Same representation and code as `banded` otherwise; oracle-pinned.
...
impl BitBoard {
    ...
    /// Column-major hidden singles (columns), NO locked candidates.
...
/// `banded-nolc` existence probe.
pub struct Probe {
    base: BitBoard,
}

impl UniqProber for Probe {
    const NAME: &'static str = "banded-nolc";
