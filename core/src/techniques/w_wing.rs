...
mod tests {
    ...
    fn finds_w_wing_with_shared_peer() {
        ...
        let mut b = crate::board::Board::empty();
        let x = 0;   // R1C1
        let y = 18 + 8; // R3C9 = row 3 col 9 = 2*9 + 8 = 26
        let _ = (x, y);
        let x = 0;
        let y = 26;
        // X = {1, 2}
        for d in [3u8,4,5,6,7,8,9] { b.eliminate(x, d); }
        // Y = {1, 2}
        for d in [3u8,4,5,6,7,8,9] { b.eliminate(y, d); }
        // Strong link on 2 in row 2 → digit 2 only at R2C1 (9) and R2C9 (17)
        for c in 1..=7 { b.eliminate(9 + c, 2); }
        let steps = find_all(&b);
        let w = steps
            .iter()
            .find(|s| s.technique == TechniqueKind::WWing)
        ...
        let r1c9 = 8;
        assert!(
            w.deductions.iter().any(|d| matches!(d, Deduction::Eliminate { cell, digit: 1 } if *cell == r1c9)),
